...
export default function Stock() {
  ...
  const handleRemovePack = (packId, packName) => {
    if (window.confirm(`Supprimer "${packName}" ?`)) {
      removePack(packId)
    }
  }
  ...
  return (
    <div className="p-4 space-y-5 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">Gestion des stocks</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">{packs.length} packs configurés</p>
        </div>
      ...
      {showAddForm && (
        <div className="card p-5 animate-scale-in">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Nouveau Pack</h3>
            <button onClick={() => setShowAddForm(false)} className="btn btn-ghost p-1">
              <X className="w-4 h-4" />
            </button>
          </div>
          <form onSubmit={handleAddPack} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">Quantité</label>
              ...
              <div>
                <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">Prix (Ar)</label>
      ...
      <div className="space-y-3">
        {packs.map(pack => {
          ...
          const badgeColor = pack.quantity <= 10
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    pack.quantity <= 10 ? 'bg-rose-50 dark:bg-rose-950/50' : pack.quantity <= 30 ? 'bg-amber-50 dark:bg-amber-950/50' : 'bg-brand-50 dark:bg-brand-950/50'
                  }`}>
                    <Package className={`w-5 h-5 ${
                      pack.quantity <= 10 ? 'text-rose-500 dark:text-rose-400' : pack.quantity <= 30 ? 'text-amber-500 dark:text-amber-400' : 'text-brand-500 dark:text-brand-400'
                    }`} />
                  </div>
                  <div>
                    <h3 className="font-semibold text-slate-800 dark:text-slate-200 text-sm">{pack.name}</h3>
                    <p className="text-xs text-slate-400 dark:text-slate-500">{formatAr(pack.price)} / unité</p>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => { setEditId(pack.id); setEditQty(pack.quantity.toString()) }} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 dark:text-slate-500 hover:text-brand-600 dark:hover:text-brand-400 transition">
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleRemovePack(pack.id, pack.name)} className="p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/50 text-slate-400 dark:text-slate-500 hover:text-rose-500 dark:hover:text-rose-400 transition">
              ...
              <div className="mb-3">
                <div className="flex justify-between items-center mb-1.5">
                  <span className={`badge ${badgeColor}`}>
                    {pack.quantity <= 10 ? 'Critique' : pack.quantity <= 30 ? 'Bas' : 'OK'}
                  </span>
                  <span className="text-sm font-bold text-slate-700 dark:text-slate-300 tabular-nums">
                    {pack.quantity} <span className="text-xs font-normal text-slate-400 dark:text-slate-500">unités</span>
                  </span>
                </div>
                ...
                <div className="flex gap-2">
                  {[{ val: 10, label: '+10' }, { val: 50, label: '+50' }, { val: 100, label: '+100' }].map(btn => (
                    <button key={btn.val} onClick={() => updatePackQuantity(pack.id, pack.quantity + btn.val)}
                      className="flex-1 flex items-center justify-center gap-1 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-brand-50 dark:hover:bg-brand-900/30 text-slate-600 dark:text-slate-400 hover:text-brand-700 dark:hover:text-brand-400 text-xs font-semibold transition border border-slate-100 dark:border-slate-700 hover:border-brand-200 dark:hover:border-brand-700">
      ...
      {packs.length === 0 && (
        <div className="card p-12 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">Aucun pack configuré</p>
          <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">Ajoutez votre premier pack pour commencer</p>
        </div>
      )}
    </div>
  )
